...
# For DIA-NN
diann_report_location <- here("data/search_fp_semi_nterm_acetyl_all_samples_3/dia-quant-output/report.tsv")  # Can be .parquet or .tsv
...
# Toggle: run imputation diagnostic plots (can be slow for large datasets)
run_imputation_diagnostics <- FALSE

# Optional guardrail: skip cached imputation plots if TSV is large (in MB)
...
rds_path_data <- file.path(rds_dir, paste0(pre_fix, search_engine, "_data.rds"))
...
# For other search engines, load external annotation file
  experimental_design <- read_delim(location_annotation) %>%
    ...
    # exclude samples not part of the final results/search
    filter(
      sample %in% str_subset(colnames(df_from_search), paste0("^", instrument))
    )

# Load protein to gene mapping with caching (aligned to data_prep_prelim outputs)
rds_path_prot2gene <- file.path(rds_dir, paste0(pre_fix, search_engine, "_prot2gene.rds"))
...
if (file.exists(rds_path_prepared)) {
  ...
  if (search_engine != "fragpipe_tmt") {
    ...
    write_tsv(imputation_summary_table, 
           file.path(here("results"), paste0(pre_fix, "_imputation_summary_table.tsv")))
...
if (!exists("imputation_plots_generated") || !isTRUE(imputation_plots_generated)) {
  ...
  imp_sum_path <- file.path(here("results"), paste0(pre_fix, "_imputation_summary_table.tsv"))
