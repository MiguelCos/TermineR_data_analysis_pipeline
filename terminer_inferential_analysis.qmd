...
# EXPERIMENT-SPECIFIC: define DA scope for your study.
# Define DA scope as: always semi-specific; if N-term patterns provided, require those too.
interesting_specificity <- c("semi_Nterm", "semi_Cterm")  # e.g., c("semi_Nterm")
interesting_nterm_modif <- c("n", "Acetyl")  # e.g., c("Dimethyl")
...
#| cache: true
# EXPERIMENT-SPECIFIC: ensure this matches the exploratory output prefix.
# Load results from exploratory analysis
exploratory_results <- read_rds(file.path(rds_dir, "pre_search_exploratory_results.rds"))
...
```{r define_contrasts}
#| cache: false
# EXPERIMENT-SPECIFIC: define contrasts using your condition names.
...
#| cache: false
# EXPERIMENT-SPECIFIC: choose which datasets to analyze in parallel.
...
datasets_to_analyze <- c("protnorm_with_lost", "non_protnorm", "protein_abund_norm")
...
run_limma_analysis <- function(se, dataset_name, annotation_df = NULL) {
  ...
  bio_replicates <- colData(se)$bio_replicate
...
if (!file.exists(file.path(rds_dir, paste0(pre_fix, "_go_enrichment_results.rds")))) {
  ...
  if (length(go_results) > 0) {
    write_rds(go_results, file.path(rds_dir, paste0(pre_fix, "_go_enrichment_results.rds"))
  }
} else {
  message("Loading pre-computed GO enrichment results... (corroborate these were run on the same DA results!)")
  go_results <- read_rds(file.path(rds_dir, paste0(pre_fix, "_go_enrichment_results.rds")))
}
```
...
if (!file.exists(file.path(rds_dir, paste0(pre_fix, "_go_cc_mapping.rds")))) {
  message("No pre-computed GO CC mapping found. Running GroupGO...")

  # Simple GroupGO for each dataset
  ...
  for (ds in names(limma_results_list)) {
    ...
    # Get all genes from limma results
    all_genes <- limma_results_list[[ds]]$results %>%
      dplyr::filter(!is.na(protein)) %>%
      dplyr::pull(protein) %>%
      unique()
    ...
    groupgo_cc <- tryCatch({
      clusterProfiler::groupGO(
        gene = all_genes,
        OrgDb = org.Hs.eg.db, # change to other organism if needed
        keyType = "UNIPROT",
        ont = "CC",
        level = 3,
        readable = TRUE
  ...
  write_rds(groupgo_results, file.path(rds_dir, paste0(pre_fix, "_go_cc_mapping.rds")))

} else {
  message("Loading pre-computed GO CC mapping... (corroborate these were run on the same DA results!)")
  groupgo_results <- read_rds(file.path(rds_dir, paste0(pre_fix, "_go_cc_mapping.rds")))
}

# Build gene to GO CC mapping table
go_cc_mapping <- bind_rows(
  groupgo_results$protnorm_with_lost@result,
  groupgo_results$non_protnorm@result
) %>% 
  distinct() %>%
  dplyr::filter(
    Count >= 2
  ...
  # collapse GO IDs and Descriptions per gene
  group_by(gene) %>%
  summarise(
...
for (ds in names(limma_results_list)) {
  ...
  result_table <- limma_results_list[[ds]]$results %>%
    ...
    # add GroupGO info if available
    left_join(go_cc_mapping, by = c("gene" = "gene")) %>%
      mutate(
        extracellular_or_cytoplasm = case_when(
          is.na(extracellular_or_cytoplasm) ~ "other/various",
          TRUE ~ extracellular_or_cytoplasm
        )) 
...
if (exists("all_results_combined")) {
  ...
  all_results_combined <- all_results_combined %>%
    ...
    left_join(
      ...
      # EXPERIMENT-SPECIFIC: set protein annotation path (Uniprot TSV for your proteome).
      protein_annotation <- read_tsv(
        "Z:/0_people/mcosenza/fastas_and_organims_annotations/org_annotations/human/uniprotkb_proteome_UP000005640_AND_mode_2025_08_13.tsv"
      ) %>%
  ...
  write_tsv(
    all_results_combined %>%
      dplyr::select(nterm_modif_peptide, contrast, logFC, AveExpr, t, P.Value, adj.P.Val, 
             regulation, dataset, everything()) %>%
      # already joined above; avoid re-joining to prevent name clashes (.x/.y)
      mutate(
        extracellular_or_cytoplasm = dplyr::coalesce(extracellular_or_cytoplasm, "other/various")
      ) %>%
